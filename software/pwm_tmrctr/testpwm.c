...
int main() {
	...
	if (status != XST_SUCCESS) 	{
		...
		xil_printf("The initialization process failed.");
		exit(XST_FAILURE);
	}
	...
	// start the PWM timer and kick off the processing by enabling the Microblaze interrupt
	...
	PMDIO_LCD_setcursor(1,0);
	PMDIO_LCD_wrstring("ECE544 Project 2");
	PMDIO_LCD_setcursor(2,0);
...
void FIT_Handler(void) {
	...
	_HWDET_register high_reg = HIGH;
	_HWDET_register low_reg = LOW;
	...
	hw_high_count = HWDET_get_count(high_reg);
	hw_low_count  = HWDET_get_count(low_reg);
